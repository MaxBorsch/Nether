...
public class ConstantSurfaceTemperatureProvider implements FacetProvider {
    private static final int SAMPLE_RATE = 4;

    private SubSampledNoise temperatureNoise;
    private float temperatureAverage;

    public ConstantSurfaceTemperatureProvider (float temperatureAverage) {
    ...
    public void process(GeneratingRegion region) {
        ...
        for (int i = 0; i < noise.length; ++i) {
            noise[i] = TeraMath.clamp(temperatureAverage + (noise[i] * 0.1f - 0.05f));
        }
